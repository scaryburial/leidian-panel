...
xui_update_status_file="${XUI_UPDATE_STATUS_FILE:-/etc/ui3344/update-status.json}"
...
prompt_and_setup_ssl() {
    ...
    case "$ssl_choice" in
        ...
        2)
            ...
            if [[ $release == "alpine" ]]; then
                rc-service ui3344 restart > /dev/null 2>&1
            else
                systemctl restart ui3344 > /dev/null 2>&1
            fi
...
config_after_update() {
    ...
    echo -e "${yellow}ui3344 settings:${plain}"
    ${xui_folder}/ui3344 setting -show true
    ${xui_folder}/ui3344 migrate

    # Properly detect empty cert by checking if cert: line exists and has content after it
    local existing_cert=$(${xui_folder}/ui3344 setting -getCert true 2> /dev/null | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
...
# by invoking the freshly downloaded ui3344 CLI. IP Limit is load-bearing on
...
update_ui3344() {
    ...
    if [ -f "${xui_folder}/ui3344" ]; then
        current_xui_version=$(${xui_folder}/ui3344 -v)
        echo -e "${green}Current ui3344 version: ${current_xui_version}${plain}"
    else
        _fail "ERROR: Current ui3344 version: unknown"
    fi

    echo -e "${green}Downloading new ui3344 version...${plain}"
    ...
    if [[ ! -s ${xui_folder}-linux-$(arch).tar.gz ]]; then
        ...
        _fail "ERROR: Downloaded ui3344 release archive is empty, please be sure that your server can access GitHub"
    fi
    ...
    if [[ "${sidecar_code}" == "200" ]]; then
        ...
        _fail "ERROR: Failed to download the checksum for ui3344-linux-$(arch).tar.gz (HTTP ${sidecar_code})"
    fi

    if [[ -e ${xui_folder}/ ]]; then
        echo -e "${green}Stopping ui3344...${plain}"
        if [[ $release == "alpine" ]]; then
            if [ -f "/etc/init.d/ui3344" ]; then
                rc-service ui3344 stop > /dev/null 2>&1
                rc-update del ui3344 > /dev/null 2>&1
                echo -e "${green}Removing old service unit version...${plain}"
                rm -f /etc/init.d/ui3344 > /dev/null 2>&1
            else
                rm ui3344-linux-$(arch).tar.gz -f > /dev/null 2>&1
                _fail "ERROR: ui3344 service unit not installed."
            fi
        else
            if [ -f "${xui_service}/ui3344.service" ]; then
                systemctl stop ui3344 > /dev/null 2>&1
                systemctl disable ui3344 > /dev/null 2>&1
                echo -e "${green}Removing old systemd unit version...${plain}"
                rm ${xui_service}/ui3344.service -f > /dev/null 2>&1
                systemctl daemon-reload > /dev/null 2>&1
            else
                rm ui3344-linux-$(arch).tar.gz -f > /dev/null 2>&1
                _fail "ERROR: ui3344 systemd unit not installed."
            fi
        fi
        ...
        echo -e "${green}Removing old ui3344 version...${plain}"
        rm ${xui_folder} -f > /dev/null 2>&1
        rm ${xui_folder}/ui3344.service -f > /dev/null 2>&1
        rm ${xui_folder}/ui3344.service.debian -f > /dev/null 2>&1
        rm ${xui_folder}/ui3344.service.arch -f > /dev/null 2>&1
        rm ${xui_folder}/ui3344.service.rhel -f > /dev/null 2>&1
        rm ${xui_folder}/ui3344 -f > /dev/null 2>&1
        rm ${xui_folder}/ui3344.sh -f > /dev/null 2>&1
        ...
        rm ui3344-linux-$(arch).tar.gz -f > /dev/null 2>&1
        _fail "ERROR: ui3344 not installed."
    fi

    echo -e "${green}Installing new ui3344 version...${plain}"
    tar zxvf ui3344-linux-$(arch).tar.gz > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        rm ui3344-linux-$(arch).tar.gz -f > /dev/null 2>&1
        _fail "ERROR: Failed to extract the ui3344 release archive -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the update again"
    fi
    rm ui3344-linux-$(arch).tar.gz -f > /dev/null 2>&1
    cd ui3344 > /dev/null 2>&1
    if [[ $? -ne 0 || ! -s ui3344 ]]; then
        _fail "ERROR: Extracted ui3344 archive is missing the ui3344 binary -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the update again"
    fi
    chmod +x ui3344 > /dev/null 2>&1
    ...
    chmod +x ui3344 bin/xray-linux-$(arch) > /dev/null 2>&1
    ...
    if [[ ! -s "${xui_script_temp}" ]]; then
        rm -f "${xui_script_temp}"
        _fail "ERROR: Downloaded ui3344.sh script is empty, please be sure that your server can access GitHub"
    fi
    mv -f "${xui_script_temp}" /usr/bin/ui3344
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
        _fail "ERROR: Failed to install ui3344.sh script"
    fi

    chmod +x ${xui_folder}/ui3344.sh > /dev/null 2>&1
    chmod +x /usr/bin/ui3344 > /dev/null 2>&1
    mkdir -p /var/log/ui3344 > /dev/null 2>&1
    ...
    if [[ $release == "alpine" ]]; then
        ...
        if [[ ! -s "${xui_rc_temp}" ]]; then
            rm -f "${xui_rc_temp}"
            _fail "ERROR: Downloaded startup unit ui3344.rc is empty, please be sure that your server can access GitHub"
        fi
        mv -f "${xui_rc_temp}" /etc/init.d/ui3344
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
            _fail "ERROR: Failed to install startup unit ui3344.rc"
        fi
        chmod +x /etc/init.d/ui3344 > /dev/null 2>&1
        chown root:root /etc/init.d/ui3344 > /dev/null 2>&1
        rc-update add ui3344 > /dev/null 2>&1
        rc-service ui3344 start > /dev/null 2>&1
    else
        if [ -f "ui3344.service" ]; then
            echo -e "${green}Installing systemd unit...${plain}"
            if ! _install_xui_service_unit "ui3344.service" "false"; then
                echo -e "${red}Failed to copy ui3344.service${plain}"
                exit 1
            fi
        else
        ...
        systemctl daemon-reload > /dev/null 2>&1
        systemctl enable ui3344 > /dev/null 2>&1
        systemctl start ui3344 > /dev/null 2>&1
    fi
    ...
    echo -e "${green}ui3344 ${tag_version}${plain} updating finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
